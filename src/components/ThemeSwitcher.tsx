...
export default function ThemeSwitcher() {
  ...
  return (
    ...
    >
      ...
      <FiSun
        className={`absolute transition-opacity duration-300 ${resolvedTheme === 'default' ? 'opacity-0' : 'opacity-100'}`}
        style={{ width: '20px', height: '20px' }}
      />
      <FiMoon
        className={`absolute transition-opacity duration-300 ${resolvedTheme === 'alternate' ? 'opacity-0' : 'opacity-100'}`}
        style={{ width: '20px', height: '20px' }}
      />
    </button>
  );
